# Project Naming / Rebranding Audit

**Status:** naming study only — **no rename performed**  
**Generated:** 2026-09-20T02:26:30Z  
**Current product label:** MegaBrain (also CursorSKILLS repo folder)  
**Personal profile label to preserve:** GaabType  

---

## 1. Question

> Qual identidade representa melhor o sistema que ele se tornou hoje?

Não é só chatbot, RAG, ou Cursor skills. É:

orchestration · capabilities · providers · policy · knowledge · evidence · evaluation · telemetry · evolution · developer environment

---

## 2. MegaBrain options

| Option | Meaning | Factual fit |
|--------|---------|-------------|
| **KEEP** | Public brand = MegaBrain | Weak for serious open-source; meme/personified; "Brain" in avoid-list |
| **RENAME** | Public brand = new technical name | Strong when publishing a reusable architecture |
| **CODENAME_ONLY** | MegaBrain remains internal/codename; public = new name | Strong transition path; least disruption to author muscle memory |

**Audit recommendation (not a final pick):** prefer **RENAME** or **CODENAME_ONLY** for public surface; do **not** treat MegaBrain as obligatory public brand. GaabType stays **profile**, never product.

---

## 3. Name layers

```text
PUBLIC PROJECT     → serious technical name (TBD)
REPOSITORY SLUG    → github.com/<org>/<slug>
INTERNAL CODENAME  → MegaBrain (optional keep)
PERSONAL PROFILE   → GaabType
```

---

## 4. Candidate families

See `PROJECT-NAMING-CANDIDATES.yaml` (~20 names across Infrastructure / Systems / Orchestration / Knowledge / Evolution / Neutral).

**Occupied / high collision (do not treat as free):** Evidora, Evidune, Outerloop, Runledger, Orchestron, Loopforge/ForgeLoop family, Cadence, Meridian (generic).

**Lower collision in superficial search (still UNKNOWN domain):** Caplane, Gatework, Skillplane, Evolplane, Hardgate, Policyrun, Northstack.

Adjacent space (not candidates, context): CodePlane, Chorus, Daintree, Entourage, Karajan, Plan Forge, AgentForge (occupied).

---

## 5. Criteria checklist (applied per candidate in YAML)

pronunciation · semantic_association · technical_tone · memorability · breadth · architecture_fit · personalization_risk · branding_risk · ambiguity · likely_collision · github_namespace_risk · domain_risk · package_name_fit · cli_fit  

Values: LOW | MEDIUM | HIGH | UNKNOWN — **no numeric scores, no single winner declared.**

---

## 6. Conclusion

- MegaBrain is **provisional / personal-toned** relative to current architecture maturity.
- Public identity should be **technical, short, pronounceable**, without Brain/Bot/AI-prefix cringe.
- GaabType **must not** become the public product name.
- Next execution phase should pick from shortlist after trademark/domain diligence (out of scope here).
