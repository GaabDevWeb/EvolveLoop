...
```text
CURSOR
  ├── ~/.cursor/skills          → megabrain-essential (25 symlinks)
...
This Cursor skill environment is the new operational baseline for MegaBrain skill loading via `~/.cursor/skills`.

## Commands dedupe (2026-09-20)

**Symptom:** slash menu showed each MegaBrain command twice.
...
**Note:** `/code-review` (Cursor product) ≠ `/code-reviewer` (MegaBrain) — not a duplicate.
