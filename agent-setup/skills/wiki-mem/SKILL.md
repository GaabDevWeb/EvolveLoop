...
**Fecho (`stop` / `sessionEnd`):** se houver `promote_candidates` ou file_edits de código, **append/refresh** `promote-queue.md`. **Nunca** escreve `{Projeto}/wiki/` nem `raw/`. `followup_message` vazio (sem loops MegaBrain). Promoção canónica = librarian / Agent.
